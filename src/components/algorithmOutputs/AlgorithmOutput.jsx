...
import OutputDrawer from "./OutputDrawer/OutputDrawer";
import { Button } from "antd";
import KuzuController from "../../wasm/Kuzu/controllers/KuzuController";
...
const AlgorithmOutput = ({ algorithm, response }) => {
  const [kuzuController] = useState(new KuzuController());
  ...
  useEffect(() => {
    kuzuController.initialize("inmemory", "sync");
    console.log(kuzuController.getService());
  }, []);
  ...
  const handleQueryExecute = () => {
    const { success, batchResults, message } =
      kuzuController.executeQuery(query);
    if (!success) {
      console.log("it breaks :(");
      return;
    }

    console.log("yippee", batchResults);
  };
    ...
    <OutputDrawer>
      <div
        ...
        <div className="tab-buttons">
          <button
            className={`tab-button ${activeTab === Tabs.CODE ? "active" : ""}`}
            onClick={() => setActiveTab(Tabs.CODE)}>
            Code
          </button>
          <button
            className={`tab-button ${
              activeTab === Tabs.OUTPUT ? "active" : ""
            }`}
            onClick={() => setActiveTab(Tabs.OUTPUT)}
            disabled={!Component && !genericComponent}>
            Output
          </button>
        </div>
        ...
        <div className="tab-content">
          {activeTab === Tabs.CODE ? (
            <div className="code-editor">
              ...
              />
              <button className="run-button" onClick={handleQueryExecute}>
                Run Code
              </button>
            </div>
          ) : (
